...
BEGIN
	...
	IF EXISTS(SELECT 1
			  FROM   SYSOBJECTS
			  WHERE  NAME ='GetSalesTemp'
			  )
	BEGIN
		DROP TABLE dbo.GetSalesTemp
	END
	...
	ALTER TABLE dbo.FactSales  WITH NOCHECK ADD CONSTRAINT FK_FactSales_OrderKey_DimOrder_OrderKey
							   FOREIGN KEY(OrderKey)
							   REFERENCES DimOrder(OrderKey)							   
	ALTER TABLE dbo.FactSales  WITH NOCHECK ADD CONSTRAINT FK_FactSales_RMAKey_DimRMA_RMAKey
							   FOREIGN KEY(RMAKey)
							   REFERENCES DimRMA(RMAKey)							   

	CREATE TABLE [dbo].[FactRMAs](
			[FactRMAKey]		[int] IDENTITY(1,1) NOT NULL,
			[InvoiceNumber]		[varchar](12)		NOT NULL,
			[InvoiceLine]       [varchar](2)		NOT NULL,
			[DateKey]			INT					NOT NULL,
			[TimeKey]			INT					NOT NULL,
			...
			[ExtendedPrice]			DECIMAL(18,4)		NOT NULL,
			[MarginAmount]		DECIMAL(18,4)		NOT NULL,
	CONSTRAINT PK_FactRMAs_SalesKey PRIMARY KEY CLUSTERED 
	(
		[FactRMAKey] ASC
	)	WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON) ON [PRIMARY]
	)	ON [PRIMARY]
	...
	ALTER TABLE dbo.FactRMAs  WITH NOCHECK ADD CONSTRAINT FK_FactRMAs_DateKey_DimDate_DateKey
							   FOREIGN KEY(DATEKEY)
							   REFERENCES DimDate(DateKey)

	ALTER TABLE dbo.FactRMAs  WITH NOCHECK ADD CONSTRAINT FK_FactRMAs_TimeKey_DimTime_TimeKey 
							   FOREIGN KEY(TimeKEY)
							   REFERENCES DimTime(TimeKey)

	ALTER TABLE dbo.FactRMAs  WITH NOCHECK ADD CONSTRAINT FK_FactRMAs_ItemKey_DimItem_ItemKey
							   FOREIGN KEY(ItemKEY)
							   REFERENCES DimItem(ItemKey)
	ALTER TABLE dbo.FactRMAs  WITH NOCHECK ADD CONSTRAINT FK_FactRMAs_SalesPersonKey_DimSalesPerson_SalesPersonKey 
							   FOREIGN KEY(SalesPersonKey)
							   REFERENCES DimSalesPerson(SalesPersonKey)

	ALTER TABLE dbo.FactRMAs  WITH NOCHECK ADD CONSTRAINT FK_FactRMAs_CustomerKey_DimCustomer_CustomerKey
							   FOREIGN KEY(CustomerKey)
							   REFERENCES DimCustomer(CustomerKey)
	ALTER TABLE dbo.FactRMAs  WITH NOCHECK ADD CONSTRAINT FK_FactRMAs_RMAKey_DimRMA_RMAKey
							   FOREIGN KEY(RMAKey)
							   REFERENCES DimRMA(RMAKey)							   
